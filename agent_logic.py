    ...
    with tabs[1]:
        ...
        for i, page in enumerate(result.get('pages', []), start=1):
            with st.expander(f"{i}. {page.get('final_url')}  —  {page.get('title') or ''}"):
                ...
                with sub_tabs[0]:
                    st.text_area("Extracted Text", page.get('text') or '', height=300, key=f"text_area_{i}")
                with sub_tabs[1]:
                    st.json(page.get('metadata') or {}, expanded=False, key=f"metadata_{i}")
                with sub_tabs[2]:
                    links = page.get('links') or []
                    if links:
                        st.dataframe(links, use_container_width=True, key=f"links_{i}")
                    else:
                        st.info("No links found.")
                with sub_tabs[3]:
                    images = page.get('images') or []
                    if images:
                        st.dataframe(images, use_container_width=True, key=f"images_{i}")
                    else:
